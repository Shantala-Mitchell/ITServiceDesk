...
function updateSeenState(tks){
  ...
  tks.forEach(function(t){
    ...
    newState[t.id]=rc;
    if(!isFirstLoad){
      if(seenState[t.id]===undefined){unseen[t.id]='new';}
      else if(rc>seenState[t.id]){unseen[t.id]='reply';}
    }
  });
...
function clearUnseen(id){
  ...
  if(row)row.classList.remove('ticket-unseen');
}
...
function startSLATicker(){if(window._slaTick)clearInterval(window._slaTick);window._slaTick=setInterval(function(){var active=tickets.filter(function(t){return !t._archived;});document.getElementById('s-breached').textContent=active.filter(function(t){return getSLAInfo(t).state==='breach';}).length;document.querySelectorAll('#ticket-body tr[data-id]').forEach(function(row){var t=getTicket(row.getAttribute('data-id'));if(!t)return;var sc=row.querySelector('.sla-cell');if(sc)sc.innerHTML=slaBadge(t);var info=getSLAInfo(t);row.className=info.state==='breach'?'sla-breach':info.state==='warn'?'sla-warn':'';});if(openId){var t=getTicket(openId);if(t)updatePanelSLA(t);}},30000);}
...
function renderDash(){
  ...
  f.forEach(function(t){
    ...
    var cls=[];
    if(unseen[t.id])cls.push('ticket-unseen');
...
function openPanel(id){
  ...
  document.getElementById('p-cat').textContent  =t.cat;
...
function sendReply(){
  ...
  if(FLOW_REPLY){
    fetch(FLOW_REPLY,{
      ...
      body:JSON.stringify({spId:t.spId,ticketId:t.id,subject:'Re: ['+t.id+'] '+t.subject,toEmail:t.email,toName:t.name,replyText:text})
    })
