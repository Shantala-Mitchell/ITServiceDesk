...
function saveCategoriesToGraph(cats,callback){
  ...
  if(cachedCategoriesSpId){
    ...
    .then(function(r){
      if(r.ok&&callback)callback();
      else r.text().then(function(t){console.error('Category PATCH failed:',r.status,t);alert('Could not save categories ('+r.status+'). Check console for details.');});
    })
    ...
    graphFetch(AGENTS_URL+'/items','POST',{fields:{Title:'__categories__',ConfigData:json}})
    .then(function(r){return r.json();})
