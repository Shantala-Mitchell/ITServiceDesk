...
var cachedCategoriesSpId = null; // SharePoint item ID for the __categories__ config entry
...
function showCCSuggestions(users){
  ...
  users.forEach(function(u){
    ...
    item.addEventListener('mousedown',function(e){
      ...
      inp.value='';hideCCSuggestions();renderWatcherTags(t);
    });
...
function mergePendingReplies(tks){
  tks.forEach(function(t){
    ...
    .then(function(res){
      ...
      return graphFetch(LIST_URL+'/items/'+t.spId+'/fields','PATCH',{
        Replies:        merged,
        PendingReply:   '',
        PendingReplyTs: ''
      });
...
function autoArchive(){
  // Use the team-level config default (ARCHIVE_DAYS from config.js) so all agents
  // share the same threshold. localStorage override only applies if explicitly set by admin.
  var teamDefault=(typeof ARCHIVE_DAYS!=='undefined'?ARCHIVE_DAYS:7);
  var s=getStoredSettings();
  var archDays=(s.archivedays!==undefined&&s.archivedays!==null&&s.archivedays!=='')?parseInt(s.archivedays):teamDefault;
  if(isNaN(archDays))archDays=teamDefault;
...
function showColumnFilter(col,anchorEl){
  ...
  if(col==='category'){
    ...
    filterOpts=[{v:'',l:'All statuses'},{v:'open',l:'Open'},{v:'in-progress',l:'In progress'},{v:'on-hold',l:'On hold'},{v:'closed',l:'Closed'}];
...
function sendReply(){
  ...
  if(FLOW_REPLY){
    ...
    .then(function(res){if(!res.ok)throw new Error('Flow returned '+res.status);t.replies=t.replies||[];t.replies.push(r);appendAudit(t,'Reply sent to requester','via email');saveRepliesWithRetry(t);persist(t);openPanel(t.id);document.getElementById('reply-status').textContent='\u2713 Reply sent';})
    ...
    t.replies=t.replies||[];t.replies.push(r);appendAudit(t,'Reply saved','no email — add Flow 4 URL in Settings');persist(t);openPanel(t.id);
...
function loadAgentsFromGraph(forceRefresh){
  ...
  .then(function(items){
    // Separate the __categories__ config item from real agent entries
    var catItem=items.find(function(i){return(i.fields.Title||'').toLowerCase()==='__categories__';});
    var agentItems=items.filter(function(i){return(i.fields.Title||'').toLowerCase()!=='__categories__';});
    // Load categories from SharePoint if the config item exists
    cachedCategoriesSpId=catItem?catItem.id:null;
    if(catItem&&catItem.fields.ConfigData){
      try{
        var loaded=JSON.parse(catItem.fields.ConfigData);
        if(Array.isArray(loaded)&&loaded.length) CATEGORIES=loaded;
      }catch(e){}
    }
    var agents=agentItems.map(function(i){
...
function saveCategoriesToGraph(cats,callback){
  ...
  if(cachedCategoriesSpId){
    // Update existing __categories__ item
    graphFetch(AGENTS_URL+'/items/'+cachedCategoriesSpId+'/fields','PATCH',{ConfigData:json})
    .then(function(r){if(r.ok&&callback)callback();})
    .catch(function(e){console.error('Category save failed:',e);alert('Could not save categories — please try again.');});
  } else {
    // Create the __categories__ item for the first time
    graphFetch(AGENTS_URL+'/items','POST',{fields:{Title:'__categories__',Role:'config',ConfigData:json}})
    .then(function(r){return r.json();})
    .then(function(data){cachedCategoriesSpId=data.id;if(callback)callback();})
    .catch(function(e){console.error('Category create failed:',e);alert('Could not save categories — check that the ConfigData column exists in your Agents list.');});
  }
}
...
function renderCategoryList(){
  ...
  if(!CATEGORIES.length){
    list.innerHTML='<div style="font-size:12px;color:var(--muted);padding:8px 0;">No categories defined. Add one below.</div>';
    return;
  }
  CATEGORIES.forEach(function(cat,i){
    ...
    nameDiv.style.flex='1';nameDiv.style.fontSize='13px';nameDiv.textContent=cat;
    row.appendChild(nameDiv);
    // Up button
    var upBtn=document.createElement('button');
    upBtn.className='btn sm';upBtn.innerHTML='&#8593;';upBtn.title='Move up';
    if(i===0)upBtn.disabled=true;
    upBtn.addEventListener('click',function(){
      if(i===0)return;
      var tmp=CATEGORIES[i-1];CATEGORIES[i-1]=CATEGORIES[i];CATEGORIES[i]=tmp;
      renderCategoryList();
    });
    row.appendChild(upBtn);
    // Down button
    var dnBtn=document.createElement('button');
    dnBtn.className='btn sm';dnBtn.innerHTML='&#8595;';dnBtn.title='Move down';
    if(i===CATEGORIES.length-1)dnBtn.disabled=true;
    ...
    rmBtn.className='rm-btn';rmBtn.innerHTML='&#10005;';rmBtn.setAttribute('aria-label','Remove '+cat);
    (function(idx,name){
      rmBtn.addEventListener('click',function(){
        if(!confirm('Remove "'+name+'"? Existing tickets with this category keep their value.'))return;
...
function addCategory(){
  ...
  if(CATEGORIES.indexOf(val)!==-1){err.textContent='That category already exists.';err.style.display='block';return;}
  CATEGORIES.push(val);
  inp.value='';
  renderCategoryList();
}
...
function saveCategories(){
  saveCategoriesToGraph(CATEGORIES,function(){
...
function loadSettingsUI(){
  ...
  renderCategoryList();
  // Force refresh from SharePoint, then re-render both lists with fresh data
  loadAgentsFromGraph(true).then(function(agents){
    renderAgentList(agents);
    renderCategoryList(); // re-render in case categories changed in SharePoint since startup
  });
  ...
  var teamDefault=(typeof ARCHIVE_DAYS!=='undefined'?ARCHIVE_DAYS:7);
  document.getElementById('s-archivedays').value=(s.archivedays!==undefined&&s.archivedays!==null)?s.archivedays:teamDefault;
...
function saveSettings(){
  ...
  var _archTeamDefault=(typeof ARCHIVE_DAYS!=='undefined'?ARCHIVE_DAYS:7);
  s.archivedays=document.getElementById('s-archivedays').value===''?_archTeamDefault:Math.max(0,parseInt(document.getElementById('s-archivedays').value)||0);
...
function bindEvents(){
  ...
  var replyOriginalBtn=document.getElementById('btn-reply-original');
  if(replyOriginalBtn) replyOriginalBtn.addEventListener('click',openCompose);
