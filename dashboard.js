...
function saveCategoriesToGraph(cats,callback){
  ...
  if(cachedCategoriesSpId){
    ...
    .then(function(r){
      if(r.ok){
        ...
        r.text().then(function(body){
          ...
          alert('Could not save categories ('+r.status+').\n\nMake sure the ConfigData column in your Agents list is set to Multiple lines of text — not Single line of text.');
        });
      }
    })
    .catch(function(e){console.error('Category save failed:',e);alert('Could not save categories — please try again.');});
  } else {
    // ── First save: create item with Title only, then PATCH ConfigData ───
    ...
    .then(function(data){
      ...
      // Now save the category data to ConfigData
      return graphFetch(AGENTS_URL+'/items/'+data.id+'/fields','PATCH',{ConfigData:json});
    })
    .then(function(r){
      if(r.ok){
        if(callback)callback();
        ...
        r.text().then(function(body){
          ...
          alert('Item created but ConfigData save failed ('+r.status+').\n\nCheck:\n1. ConfigData column exists in your Agents list\n2. It is set to Multiple lines of text (not Single line)');
        });
      }
    })
    .catch(function(e){
