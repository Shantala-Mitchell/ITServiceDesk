...
function getToken(){
  ...
  .catch(function(e){
    if(e instanceof msal.InteractionRequiredAuthError||e.name==='InteractionRequiredAuthError'||(e.errorCode&&e.errorCode==='interaction_required')){
      ...
      // Give the redirect 3 seconds to navigate — if it doesn't, reject so the app
      // falls back gracefully rather than hanging forever on "Loading..."
      return new Promise(function(_,reject){setTimeout(function(){reject(new Error('Token redirect timeout'));},3000);});
    }
    throw e;
  });
}
...
function openPanel(id){
  ...
  if(!replies.length){
    ...
    replies.forEach(function(r,i){
      ...
      var replyIcon=
        '<button class="rb-reply-btn" data-mode="reply" title="Reply" aria-label="Reply">'+
          '<svg viewBox="0 0 16 16" width="11" height="11" aria-hidden="true"><path d="M7 4 L3 8 L7 12 M3 8 L11 8 Q14 8 14 12 L14 13" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg>'+
        '</button>'+
        '<button class="rb-reply-btn rb-reply-btn--text" data-mode="reply-all" title="Reply All" aria-label="Reply All">Reply All</button>'+
        '<button class="rb-reply-btn rb-reply-btn--text" data-mode="forward" title="Forward" aria-label="Forward">Forward</button>';
...
function loadSettingsUI(){
  ...
  renderCategoryList();
  // Refresh agents from SharePoint
  loadAgentsFromGraph(true).then(function(agents){
    renderAgentList(agents);
  });
  // Refresh categories from file independently
  loadCategoriesFromFile().then(function(){
    renderCategoryList();
  });
...
function bindEvents(){
  ...
  document.getElementById('reply-thread').addEventListener('click',function(e){
    ...
    if(rbtn){
      e.stopPropagation();
      var mode=rbtn.getAttribute('data-mode')||'reply';
      setReplyMode(mode);
      openCompose();
      return;
    }
