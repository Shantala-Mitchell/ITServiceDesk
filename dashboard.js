...
function sendReply(){
  ...
  if(FLOW_REPLY){
    ...
    .then(function(res){if(!res.ok)throw new Error('Flow returned '+res.status);t.replies=t.replies||[];t.replies.push(r);appendAudit(t,'Reply sent to requester','via email');if(t.status==='in-progress'||t.status==='open'){applyStatusChange(t,'customer-waiting');appendAudit(t,'Status auto-changed','In Progress → Customer Waiting');}saveRepliesWithRetry(t);persist(t);openPanel(t.id);document.getElementById('reply-status').textContent='\u2713 Reply sent';})
    ...
    t.replies=t.replies||[];t.replies.push(r);appendAudit(t,'Reply saved','no email — add Flow 4 URL in Settings');if(t.status==='in-progress'||t.status==='open'){applyStatusChange(t,'customer-waiting');appendAudit(t,'Status auto-changed','In Progress → Customer Waiting');}persist(t);openPanel(t.id);
