...
function searchCCUsers(query){
  getToken().then(function(token){
    return fetch('https://graph.microsoft.com/v1.0/users?$filter=startswith(displayName,\''+encodeURIComponent(query)+'\') or startswith(mail,\''+encodeURIComponent(query)+'\')&$select=displayName,mail&$top=8',{
      headers:{'Authorization':'Bearer '+token,'Accept':'application/json'}
    });
  }).then(function(r){return r.json();})
  .then(function(data){
    if(data.error){
      console.error('CC search error:',data.error.code,data.error.message);
      return;
    }
