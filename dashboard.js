...
function searchCCUsers(query){
  getToken().then(function(token){
    return fetch('https://graph.microsoft.com/v1.0/users?$search="displayName:'+encodeURIComponent(query)+'"&$select=displayName,mail&$top=8&$orderby=displayName',{
      headers:{'Authorization':'Bearer '+token,'ConsistencyLevel':'eventual','Accept':'application/json'}
    });
  }).then(function(r){return r.json();})
  .then(function(data){showCCSuggestions((data.value||[]).filter(function(u){return u.mail;}));})
