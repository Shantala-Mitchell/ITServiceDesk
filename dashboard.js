...
/* ═══════════ PENDING REPLY MERGE ═══════════ */
var _pendingMerged={}; // ticketId → text already merged this session (prevents double-merge on rapid reloads)

function mergePendingReplies(tks){
  tks.forEach(function(t){
    if(!t.pendingReply||!t.spId)return;
    // Skip if this exact pending reply has already been merged in this session
    if(_pendingMerged[t.id]===t.pendingReply)return;
    _pendingMerged[t.id]=t.pendingReply; // mark immediately before async PATCH
    ...
    // Sort by timestamp to keep conversation in order
    t.replies.sort(function(a,b){return new Date(a.ts)-new Date(b.ts);});
    // Add audit entry
    appendAudit(t,'Reply received from requester','via email');
    // Persist merged replies and clear pending fields in SharePoint
    var merged=JSON.stringify(t.replies);
    t.pendingReply='';
    t.pendingReplyTs='';
    graphFetch(LIST_URL+'/items/'+t.spId+'/fields','PATCH',{
      Replies:        merged,
      PendingReply:   '',
      PendingReplyTs: ''
    }).catch(function(e){
      console.error('Pending reply merge failed for '+t.id+':',e);
      delete _pendingMerged[t.id]; // allow retry on next load if PATCH failed
    });
  });
}
...
function openPanel(id){
  ...
  if(!replies.length){
    ...
    replies.forEach(function(r,i){
      ...
      var who=r.from==='agent'?'You (IT Support)':'Staff member';
      ...
      // Collapsed items show a text preview and chevron
      if(!isLatest){
        var preview=r.text.replace(/\s+/g,' ').trim().substring(0,120);
        hdr.innerHTML+='<span class="rb-preview">'+esc(preview)+'</span><span class="rb-chev">&#9660;</span>';
      }
...
function sendReply(){
  ...
  if(FLOW_REPLY){
    ...
    .then(function(res){if(!res.ok)throw new Error('Flow returned '+res.status);t.replies=t.replies||[];t.replies.push(r);appendAudit(t,'Reply sent to requester','via email');persist(t);openPanel(t.id);document.getElementById('reply-status').textContent='\u2713 Reply sent';})
...
function bindEvents(){
  ...
  // Reply accordion — click to expand/collapse (latest message always stays open)
  document.getElementById('reply-thread').addEventListener('click',function(e){
    var rb=e.target.closest('.rb');
    if(!rb||rb.classList.contains('latest'))return;
