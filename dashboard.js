...
function populateCategoryDropdown(){
  var sel=document.getElementById('fd-category');
  var current=sel.value;
  while(sel.options.length>1)sel.remove(1);
  // Use categories from tickets actually in the system, supplemented by CATEGORIES list
  var seen={};
  tickets.forEach(function(t){if(t.cat)seen[t.cat]=true;});
...
function populateAssignedDropdown(){
  var sel=document.getElementById('fd-assigned');
  var current=sel.value;
  while(sel.options.length>3)sel.remove(3); // keep All, Me, Unassigned
  if(cachedAgents){
    cachedAgents.forEach(function(a){
      var opt=document.createElement('option');
      opt.value=a.email.toLowerCase();
      opt.textContent=a.email.split('@')[0];
      sel.appendChild(opt);
    });
  }
  sel.value=current||'';
...
function bindEvents(){
  ...
  // Tickets
  // Filter dropdowns
  document.getElementById('fd-category').addEventListener('change',function(){dropFilter.cat=this.value;this.className='filter-sel'+(this.value?' active':'');renderDash();});
  document.getElementById('fd-priority').addEventListener('change',function(){dropFilter.pri=this.value;this.className='filter-sel'+(this.value?' active':'');renderDash();});
  document.getElementById('fd-status').addEventListener('change',function(){dropFilter.sta=this.value;this.className='filter-sel'+(this.value?' active':'');renderDash();});
  document.getElementById('fd-assigned').addEventListener('change',function(){dropFilter.asg=this.value;this.className='filter-sel'+(this.value?' active':'');renderDash();});
  document.getElementById('btn-clear-dropdowns').addEventListener('click',function(){
    dropFilter={cat:'',pri:'',sta:'',asg:''};
    ['fd-category','fd-priority','fd-status','fd-assigned'].forEach(function(id){
      var el=document.getElementById(id);el.value='';el.className='filter-sel';
    });
    renderDash();
  });
