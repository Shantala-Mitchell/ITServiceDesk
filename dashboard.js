...
function setReplyMode(mode){
  ...
  if(mode==='forward'){
    ...
    // Reply: no CC — hide the CC row so it's clear only the requester receives it
    if(fwdRow)fwdRow.style.display='none';
    if(staticToRow)staticToRow.style.display='';
    if(ccRow)ccRow.style.display='none';
    if(ta)ta.placeholder='Type your reply — this will be emailed to the requester only…';
  } else {
    // Reply All: show CC row so agent can see and manage watchers
    if(fwdRow)fwdRow.style.display='none';
    if(staticToRow)staticToRow.style.display='';
    if(ccRow)ccRow.style.display='';
    if(ta)ta.placeholder='Type your reply — this will be emailed to the requester and all CC recipients…';
  }
}
...
function loadAgentsFromGraph(forceRefresh){
  ...
  .then(function(items){
    ...
    if(!_categoryDirty){
      if(catItem&&catItem.fields.CategoryConfig){
        try{
          var loaded=JSON.parse(catItem.fields.CategoryConfig);
...
function saveCategoriesToGraph(cats,callback){
  ...
  if(cachedCategoriesSpId){
    ...
    graphFetch(AGENTS_URL+'/items/'+cachedCategoriesSpId+'/fields','PATCH',{CategoryConfig:json})
    .then(function(r){
      if(r.ok){
        if(callback)callback();
      } else {
        r.text().then(function(body){
          console.error('Category PATCH failed:',r.status,body);
          alert('Could not save categories ('+r.status+').\n\nMake sure the CategoryConfig column in your Agents list is set to Multiple lines of text — not Single line of text.');
        });
      }
    })
    .catch(function(e){console.error('Category save failed:',e);alert('Could not save categories — please try again.');});
  } else {
    // ── First save: create item with Title only, then PATCH CategoryConfig ───
    // Splitting into two steps avoids a 400 caused by custom column issues
    // on initial item creation.
    graphFetch(AGENTS_URL+'/items','POST',{fields:{Title:'__categories__'}})
    .then(function(r){
      if(!r.ok) return r.text().then(function(body){throw new Error('Create failed ('+r.status+'): '+body);});
      return r.json();
    })
    .then(function(data){
      if(!data.id) throw new Error('No id in create response');
      cachedCategoriesSpId=data.id;
      // Now save the category data to CategoryConfig
      return graphFetch(AGENTS_URL+'/items/'+data.id+'/fields','PATCH',{CategoryConfig:json});
    })
    .then(function(r){
      if(r.ok){
        if(callback)callback();
        ...
        r.text().then(function(body){
          console.error('Category data save failed:',r.status,body);
          alert('Item created but CategoryConfig save failed ('+r.status+').\n\nCheck:\n1. CategoryConfig column exists in your Agents list\n2. It is set to Multiple lines of text (not Single line)');
        });
      }
    })
    .catch(function(e){
      console.error('Category create failed:',e);
      alert('Could not save categories: '+e.message);
    });
  }
}
...
function loadSettingsUI(){
  ...
  loadAgentsFromGraph(true).then(function(agents){
    ...
    // Note: intentionally not re-rendering categories here — async SP refresh
    // must not overwrite in-progress category edits.
  });
