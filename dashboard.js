...
function searchCCUsers(query){
  console.log('[CC Search] Searching for:',query);
  getToken().then(function(token){
    var url='https://graph.microsoft.com/v1.0/users?$filter=startswith(displayName,\''+query+'\')&$select=displayName,mail&$top=8';
    console.log('[CC Search] URL:',url);
    return fetch(url,{headers:{'Authorization':'Bearer '+token,'Accept':'application/json'}});
  }).then(function(r){
    console.log('[CC Search] Status:',r.status);
    return r.json();
  })
  .then(function(data){
    console.log('[CC Search] Response:',data);
    if(data.error){console.error('[CC Search] Error:',data.error.code,data.error.message);return;}
  ...
  tks.forEach(function(t){
    ...
    .then(function(res){
      ...
      if(currentTs&&currentTs!==mergedTs){
        console.log('New reply arrived during merge for '+t.id+' — will merge on next load');
        // Still save the Replies we've merged so far
...
function saveRepliesWithRetry(t,attempt){
  ...
  .then(function(){if(attempt>1)console.log('Replies saved on attempt '+attempt);})
  .catch(function(e){
    if(attempt<MAX){
...
function autoArchive(){
  var s=getStoredSettings();var archDays=s.archivedays===''||s.archivedays===undefined?7:parseInt(s.archivedays);if(isNaN(archDays))archDays=7;var days=archDays*86400000;
...
function getRptDays(){
  var val=document.getElementById('rpt-range').value;
  if(val==='ytd'){var now=new Date();var jan1=new Date(now.getFullYear(),0,1);return Math.ceil((now-jan1)/86400000)+1;}
...
function showColumnFilter(col,iconEl){
  closeColumnFilter();
  ...
  if(col==='category'){
    ...
    CATEGORIES.forEach(function(c){cats[c]=1;});
    opts=[{v:'',l:'All categories'}].concat(Object.keys(cats).sort().map(function(c){return{v:c,l:c};}));
  } else if(col==='priority'){
    opts=[{v:'',l:'All priorities'},{v:'critical',l:'\u{1F534} Critical'},{v:'high',l:'\u{1F7E0} High'},{v:'medium',l:'\u{1F7E1} Medium'},{v:'low',l:'\u{1F7E2} Low'}];
  } else if(col==='status'){
    opts=[{v:'',l:'All statuses'},{v:'open',l:'Open'},{v:'in-progress',l:'In Progress'},{v:'on-hold',l:'On Hold'},{v:'closed',l:'Closed'}];
  } else if(col==='assigned'){
    opts=[{v:'',l:'All agents'},{v:'__me__',l:'Assigned to me'},{v:'__unassigned__',l:'Unassigned'}];
    if(cachedAgents)cachedAgents.forEach(function(a){opts.push({v:a.email.toLowerCase(),l:a.email.split('@')[0]});});
  }
  ...
  opts.forEach(function(o){
    var item=document.createElement('div');
    item.className='col-filter-item'+(o.v===''?' clear-item':'')+(current===o.v?' selected':'');
  ...
  if(!replies.length){
    ...
    replies.forEach(function(r,i){
      ...
      hdr.innerHTML='<span class="rb-who">'+esc(who)+'</span><span class="rb-ts">'+fmtTime(r.ts)+'</span>'+
        '<span class="rb-preview">'+esc(preview)+'</span><span class="rb-chev">&#9660;</span>';
...
function saveSettings(){
  ...
  s.archivedays=document.getElementById('s-archivedays').value===''?7:Math.max(0,parseInt(document.getElementById('s-archivedays').value)||0);
...
function bindEvents(){
  ...
  // Reply accordion — click to expand/collapse (all items toggleable, latest starts open)
  ...
  document.querySelectorAll('th .flt[data-filtercol]').forEach(function(icon){
    icon.addEventListener('click',function(e){
      ...
      showColumnFilter(this.dataset.filtercol,this);
    });
  });
