...
var _categoryDirty      = false; // true when user has unsaved category edits — prevents SP refresh overwriting changes
...
function setReplyMode(mode){
  replyMode=mode;
  ['reply','reply-all','forward'].forEach(function(m){
    var btn=document.getElementById('btn-mode'+m.replace('-',''));
  ...
  if(mode==='forward'){
    ...
    if(ta)ta.placeholder='Type your reply — this will be emailed to the requester…';
  }
}
...
function openPanel(id){
  ...
  // Show CC recipients in the original request header
  var ccRow=document.getElementById('p-cc-row');
  var ccList=document.getElementById('p-cc-list');
  if(ccRow&&ccList){
    var watchers=getWatchers(t);
    if(watchers.length){ccList.textContent=watchers.join(', ');ccRow.style.display='';}
...
function loadAgentsFromGraph(forceRefresh){
  ...
  .then(function(items){
    ...
    // Only update CATEGORY_CONFIG if the user hasn't made unsaved edits this session
    if(!_categoryDirty){
        ...
        try{
          ...
          if(Array.isArray(loaded)&&loaded.length){
            ...
            if(typeof loaded[0]==='string'){
              CATEGORY_CONFIG=loaded.map(function(s){return{name:s,active:true};});
            } else {
              CATEGORY_CONFIG=loaded;
            }
            syncActiveCategories();
          }
        }catch(e){}
      } else {
        // No SP config yet — bootstrap from hardcoded CATEGORIES
  ...
  if(!inactive.length){
    ...
    inactive.forEach(function(cat){
      ...
      (function(name){actBtn.addEventListener('click',function(){
        ...
        if(item){item.active=true;_categoryDirty=true;}
        renderCategoryList();
      });})(cat.name);
    ...
    active.forEach(function(cat,i){
      ...
      (function(name,idx){
        ...
        deactBtn.addEventListener('click',function(){
          var item=CATEGORY_CONFIG.find(function(c){return c.name===name;});
          if(item){item.active=false;_categoryDirty=true;}
          renderCategoryList();
        });
      })(cat.name,i);
...
function saveCategories(){
  ...
  saveCategoriesToGraph(CATEGORY_CONFIG,function(){
    _categoryDirty=false; // saved — SP refresh can update CATEGORY_CONFIG again
...
function bindEvents(){
  ...
  // Categories
  // Reply mode buttons
  var modeReply=document.getElementById('btn-modereply');
  var modeReplyAll=document.getElementById('btn-modereplyall');
  var modeFwd=document.getElementById('btn-modeforward');
  if(modeReply)modeReply.addEventListener('click',function(){setReplyMode('reply');});
  if(modeReplyAll)modeReplyAll.addEventListener('click',function(){setReplyMode('reply-all');});
  if(modeFwd)modeFwd.addEventListener('click',function(){setReplyMode('forward');});
