...
function clearUnseen(id){
  console.log('[Unseen] clearUnseen called for:', id);
  _viewedTickets.add(id);
  var unseen=getUnseenStore();
  console.log('[Unseen] store before clear:', JSON.stringify(unseen));
  console.log('[Unseen] _viewedTickets has id:', _viewedTickets.has(id));
  if(unseen[id]){
    delete unseen[id];
    localStorage.setItem('itdesk_unseen',JSON.stringify(unseen));
    console.log('[Unseen] removed from store');
  } else {
    console.log('[Unseen] id was not in store — nothing to remove');
  }
  var row=document.querySelector('#ticket-body tr[data-id="'+id+'"]');
  console.log('[Unseen] DOM row found:', !!row, row?'classes: '+row.className:'');
  if(row){row.classList.remove('ticket-unseen');var dot=row.querySelector('.unseen-dot');if(dot)dot.remove();}
  console.log('[Unseen] DOM row classes after remove:', row?row.className:'n/a');
}
...
function persistToGraph(t){
  ...
  var fields={
    ...
    HoldDuration: t.holdDuration||0,
    HoldStart:    t.holdStart||'',
    Closed:       t.closed||''
  };
  console.log('[Save] persistToGraph called. spId:', t.spId, 'url:', LIST_URL+'/items/'+t.spId+'/fields');
  console.log('[Save] fields:', JSON.stringify(fields));
  return graphFetch(LIST_URL+'/items/'+t.spId+'/fields','PATCH',fields)
  .then(function(r){
    console.log('[Save] PATCH response status:', r.status, 'ok:', r.ok);
    if(!r.ok){
      return r.text().then(function(txt){
        console.error('[Save] PATCH failed:', txt);
        throw new Error('PATCH failed: '+r.status);
      });
    }
    console.log('[Save] PATCH succeeded');
  });
}
...
function renderDash(){
  ...
  f.forEach(function(t){
    ...
    var cls=[];
    var inUnseen=!!unseen[t.id];
    var inViewed=_viewedTickets.has(t.id);
    if(inUnseen&&!inViewed){cls.push('ticket-unseen');console.log('[Unseen] renderDash: adding ticket-unseen to',t.id,'unseen=',inUnseen,'viewed=',inViewed);}
