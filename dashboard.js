...
function setReplyMode(mode){
  ...
  if(mode==='forward'){
    ...
    // Reply: show CC row so agents can manually add someone, but existing
    // watchers are not auto-sent — that is Reply All's behaviour.
    ...
    // Reply All: CC row shows existing watchers, all will be sent
    if(fwdRow)fwdRow.style.display='none';
    if(staticToRow)staticToRow.style.display='';
    if(ccRow)ccRow.style.display='';
    if(ta)ta.placeholder='Type your reply…';
